...
use bouncycastle_sha3::hmac::{HMAC_SHA3_256, HMAC_SHA3_512};
use criterion::{Criterion, Throughput, criterion_group, criterion_main};
use std::hint::black_box;

fn bench_hmac_sha3_256(c: &mut Criterion) {
    ...
    group.bench_function(format!("{} bytes -- ::hashes()", big_data.len() as u64), |b| {
        b.iter(|| {
            HMAC_SHA3_256::new(&hmac_key).unwrap().mac_out(black_box(&big_data), &mut out).unwrap();
            black_box(&out);
        })
    });
    group.finish();
...
fn bench_hmac_sha3_512(c: &mut Criterion) {
    let mut data_block = [0_u8; 1024];
    rng::DefaultRNG::default().next_bytes_out(&mut data_block).unwrap();
    ...
    group.bench_function(format!("{} bytes -- ::hashes()", big_data.len() as u64), |b| {
        b.iter(|| {
            HMAC_SHA3_512::new(&hmac_key).unwrap().mac_out(black_box(&big_data), &mut out).unwrap();
            black_box(&out);
        })
    });
    group.finish();
...
criterion_group!(benches, bench_hmac_sha3_256, bench_hmac_sha3_512);
